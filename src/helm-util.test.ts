...
describe('Testing all functions in helm-util file.', () => {
   test('walkSync() - return path to the all files matching fileToFind in dir', () => {
      jest.spyOn(fs, 'readdirSync').mockImplementation((file, _) => {
         if (file == 'mainFolder')
            return ['file1', 'file2', 'folder1', 'folder2'] as unknown as fs.Dirent<Buffer<ArrayBufferLike>>[]
         if (file == path.join('mainFolder', 'folder1'))
            return ['file11', 'file12'] as unknown as fs.Dirent<Buffer<ArrayBufferLike>>[]
         if (file == path.join('mainFolder', 'folder2'))
            return ['file21', 'file22'] as unknown as fs.Dirent<Buffer<ArrayBufferLike>>[]
         return []
      })
   ...
   test('walkSync() - return empty array if no file with name fileToFind exists', () => {
      jest.spyOn(fs, 'readdirSync').mockImplementation((file, _) => {
         if (file == 'mainFolder')
            return ['file1', 'file2', 'folder2' ] as unknown as fs.Dirent<Buffer<ArrayBufferLike>>[]
         if (file == path.join('mainFolder', 'folder1'))
            return [ 'file11', 'file12' ] as unknown as fs.Dirent<Buffer<ArrayBufferLike>>[]
         if (file == path.join('mainFolder', 'folder2'))
            return [ 'file21', 'file22' ] as unknown as fs.Dirent<Buffer<ArrayBufferLike>>[]
